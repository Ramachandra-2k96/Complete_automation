...
actor_llm = Ollama(model=MODEL_NAME, request_timeout=60000, temperature=0.9)

# RAG LLM (answers questions about IonIdea)
rag_llm = Ollama(model="granite3.1-moe", request_timeout=60000, temperature=0.3)
...
def setup_rag_system(index):
    ...
    rag_system_prompt = """You are a technical data provider for IonIdea. ONLY output raw facts and precise data points.
...
- SPEAK LIKE A DATABASE: "IonIdea founded 1998. 450 employees. 37 banking clients."
- MUST include specific numbers: dates, counts, percentages, versions
- NO descriptions, explanations, context, or qualifiers
- If information is not available, say ONLY: "No data available for [specific topic]."
- NEVER say "I don't have" or "I can't provide" - you're not a person
- Answer as if reading from a technical specification sheet

VIOLATION OF THESE RULES WILL RESULT IN COMPLETE FAILURE. You MUST provide exact data only.
"""
    ...
    # Custom prompts for better control - updated for more conciseness
    context_prompt = """# CONTEXT DATA
{context_str}

## EXTREME OUTPUT RULES:
- MAXIMUM 2-3 sentences
- OUTPUT FORMAT: "[Fact 1 with numbers]. [Fact 2 with numbers]."
- REQUIRED: Exact counts, years, percentages, versions
- FORBIDDEN: Adjectives, adverbs, marketing terms, corporate language
- ONLY provide direct raw data from context
- Format like database output, not prose
- NO explanations or context setting
"""

    context_refine_prompt = """# QUERY AND CURRENT ANSWER
- Query: {query_str}
...
## EXTREME OUTPUT RULES:
- MAXIMUM 2-3 sentences
- ONLY add exact numbers, dates, counts from this context
- FORBIDDEN: Corporate language, explanations, marketing terms
- Format like database output: "IonIdea projects: 250 completed. Client retention: 85%."
- REQUIRED: Technical specifications with version numbers when applicable
- NO adjectives, qualifiers or marketing language
"""
...
## Task:
Convert to shortest possible technical query. One sentence maximum.
"""
